    ...
    const created = await prisma.$transaction(async (tx) => {
      ...
      const rows = await query(
        ...
        [
          ...
          data.email_nivel1 || "",
          data.email_nivel2 || "",
          data.default_project_number || "",
          data.default_project_client || "",
  ...
  asyncHandler(async (req, res) => {
    ...
    const rows = await query(
      ...
      UPDATE timesheets SET
        ...
        direcao = COALESCE($8, direcao),
        centro_custo = COALESCE($9, centro_custo),
        cct = COALESCE($10, cct),
        horario = COALESCE($11, horario),
        email_remetente = COALESCE($12, email_remetente),
        email_nivel1 = COALESCE($13, email_nivel1),
        email_nivel2 = COALESCE($14, email_nivel2),
        default_project_number = COALESCE($15, default_project_number),
        default_project_client = COALESCE($16, default_project_client),
        default_project_description = COALESCE($17, default_project_description),
        source_filename = COALESCE($18, source_filename),
        source_file_url = COALESCE($19, source_file_url),
        total_compensation_hours = COALESCE($20, total_compensation_hours),
        total_descanso_compensatorio_hours = COALESCE($21, total_descanso_compensatorio_hours)
      ...
      [
        ...
        data.cct ?? null,
        data.horario ?? null,
        data.email_remetente ?? null,
        data.email_nivel1 ?? null,
        data.email_nivel2 ?? null,
        data.default_project_number ?? null,
        data.default_project_client ?? null,
        data.default_project_description ?? null,
