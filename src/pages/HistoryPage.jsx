...
import { Calendar, Clock, DownloadCloud, Eye, Pencil, Trash2, Upload, User, Wallet } from "lucide-react";
...
export default function HistoryPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto w-full px-4 sm:px-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">Histórico</h2>
          <p className="text-sm text-muted-foreground">
            {tab === "timesheets" ? timesheetCountLabel : `${enjoyments.length} registo(s)`}
          </p>
        </div>
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
          <Select value={tab} onValueChange={setTab}>
            <SelectTrigger className="w-full bg-card/80 backdrop-blur border-border/60 sm:w-[320px]">
                ...
                <div key={e.id} className="bg-card border border-border rounded-xl p-5 flex flex-col sm:flex-row gap-4">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {e.reason ? (
                        <p className="text-sm text-muted-foreground mt-2 whitespace-pre-wrap">{e.reason}</p>
                      ) : null}
                    </div>
                  </div>
